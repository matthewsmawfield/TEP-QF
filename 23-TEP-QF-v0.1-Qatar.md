...
In the TEP framework, antimatter is not introduced as an independent ontological species; it is interpreted as reversed proper-time phase orientation on the second sheet of the temporal manifold. The two-sheeted structure is not an arbitrary addition; it is motivated by the standard time-orientation construction associated with Lorentzian geometry, where the scalar gradient *∇_{μ}φ* defines a local timelike direction wherever it is non-null in *g̃_{μν}*. In the TEP framework, the causal metric and scalar gradient supply the local orientation structure, while the physical identification of the two sheets with matter and antimatter is the proposed geometric completion. The branch locus *J_{br}* is the set of points where the local proper-time covector *k_{μ} = ∂_{μ}S* is null in the causal metric (*g̃^{μν} k_{μ} k_{ν} = 0*), i.e. where the orientation is not globally continuable to a single sheet. In the screened tangent limit, the observable orientation distinction is suppressed: the scalar field is locally constant over the interaction scale, the shear sector becomes unresolvable, and the effective laboratory description reduces to the single-sheeted Minkowski background. The observed reversal of additive quantum numbers is recovered as the local representation of charge conjugation. Where a particle's phase advances as *+τ̃* along the future-directed light cone, its antiparticle advances as *−τ̃* along the past-directed cone.

This reversal is a *geometric*, not algebraic, operation. It arises naturally from the two-sheeted structure of the proper-time manifold, where the "other sheet" corresponds to reversed phase orientation. The two-sheeted topology is a global property of the temporal manifold and is independent of the local density. The critical saturation scale *ρ_{T} ≈ 20 g/cm^{3}* is the Temporal Topology saturation scale at which screening effects saturate; it does not act as a boundary between the sheets. In the screened limit, where the observable shear sector *Σ_{μ}^{obs} → 0* with *A(φ)* normalized to its ambient environmental value, the observable temporal shear is suppressed and the phase-orientation distinction becomes unresolvable at the measurement scale, recovering the standard CPT-symmetric effective theory.

Self-conjugate states. For photons, the null character of the excitation suggests a natural association with the branch locus *J_{br}*, because a null covector *k_{μ}* satisfies *g̃^{μν} k_{μ} k_{ν} = 0* everywhere along the worldline, placing the excitation on the seam rather than on either sheet. More generally, self-conjugate states should correspond to sheet-symmetric configurations invariant under the deck transformation; this remains an open consistency condition for the full topological completion.

Geometric junction of the two sheets. Formally, the proper-time manifold is treated as a branched double cover with local sheets *M_{+}* and *M_{−}*, distinguished by the sign of phase accumulation along the matter-clock congruence. The sheets are connected only through a branch locus in the orientation bundle; its projection into laboratory spacetime may be a localized vertex event. The branch locus *J_{br}* is the codimension-one locus in the orientation bundle on which future-directed and past-directed orientations meet: points of *J_{br}* are those at which the local proper-time covector *k_{μ} = ∂_{μ}S* is null in the causal metric (*g̃^{μν} k_{μ} k_{ν} = 0*) and the orientation of *k_{μ}* is not globally continuable to a single sheet without crossing a branch. Charge conjugation *C* acts locally as orientation reversal across *J_{br}*: a future-directed worldline on *M_{+}* maps to a past-directed continuation on *M_{−}* with *dτ̃ → −dτ̃*. Particle–antiparticle creation and annihilation are the physical realisations of this junction: a single connected phase contour on *J_{br}* bifurcates onto opposite sheets at creation, and opposite contours recombine at annihilation. The junction is therefore not a spatial wall in laboratory coordinates; it is a topological seam in the orientation bundle, localised to events where the temporal phase field changes sheet. At the vertex, the electromagnetic coupling supplies the energy–momentum required for the incoming null phase contour to acquire two timelike, on-shell continuations. In the TEP interpretation, the branching occurs in the orientation sector: the phase contour reaches the null branch condition at *J_{br}*, while the conformal–disformal metric supplies the local causal geometry. The electromagnetic coupling is the trigger because it furnishes the energy budget that moves the covector off the null condition onto the timelike mass shell; a single sheet cannot support both future-directed and reversed-phase timelike continuations from the same null root, so the phase contour must bifurcate at *J_{br}*. This should be read as a geometric interpretation of the screened QED vertex, not as a replacement for the QED amplitude calculation. Away from *J_{br}*, each sheet supports smooth, future-directed propagation; across *J_{br}*, only the full branched-cover configuration remains globally single-valued.

Laboratory realisation: pair production. The abstract seam *J_{br}* admits a concrete laboratory identification in photon-induced pair production, *γ + Z → e^{−} + e^{+} + Z*, where a high-energy photon converts in the Coulomb field of a nucleus *Z*. The process is traced here in laboratory coordinates *(t, x)*; the sheet assignment is a property of the orientation bundle, not of the spatial direction of motion in the lab frame.

*(i) Pre-junction approach.* Before the vertex, the photon propagates on the future null cone with *k_{μ} k^{μ} = 0* in *g̃*. Its phase action *S* advances along the null generator; no timelike proper-time oscillator is yet present because the photon is massless. The covector *k_{μ} = ∂_{μ}S* is future-directed and null, placing the photon on the branch locus *J_{br}* rather than on either orientation sheet.

*(ii) Junction event.* At the interaction vertex—the spacetime event at which the electromagnetic coupling supplies sufficient energy–momentum budget to populate the electron mass shell—the local covector remains null on the photon mass shell but is no longer globally continuable on a single sheet: the connected phase contour must branch to support two timelike, on-shell continuations. This vertex is a point of *J_{br}*. It is localised to a single laboratory event (a point in *(t, x)*), not extended along a spatial hypersurface; detectors record the bifurcation only through the emergence of two timelike tracks downstream of that event.

*(iii) Bifurcation onto opposite sheets.* From the connected contour on *J_{br}* emerge two branches. The electron continues on *M_{+}* with timelike *k^{μ}*, accumulating phase as *+dτ̃* along a future-directed worldline. The positron continues on *M_{−}* with reversed proper-time orientation, *dτ̃ → −dτ̃* relative to the matter-clock congruence. Both worldlines advance forward in coordinate time *t*; the sheet distinction is geometric, not a reversal of laboratory causality. The Feynman–Stückelberg reinterpretation is recovered as the screened-laboratory projection of a past-directed proper-time orientation on *M_{−}*: the positron appears in detectors as a particle of opposite charge propagating forward in *t*, while its phase transport is reversed on the second sheet. In the screened limit, all standard QED observables—track curvature in a magnetic field, bremsstrahlung spectra, coincidence timing—are recovered; the TEP content is the geometric identification of the creation vertex with a branch point on *J_{br}*.

*(iv) Post-creation separation and entanglement.* As the pair separates, each branch carries a localized topological charge (Section 4.1) on its respective sheet. The phase contour rooted at the *J_{br}* event remains a single connected geometric object: TEP-KIN (Paper 25) identifies this as the macroscopic entanglement link between the bifurcated charges. The junction event is therefore not only the sheet transition but also the origin of the shared orientation-bundle holonomy that enforces quantum correlations between the outgoing tracks.

*(v) Annihilation as recombination.* The reverse process, *e^{−} + e^{+} → γ + γ*, is recombination on *J_{br}*: timelike contours from *M_{+}* and *M_{−}* converge at a common null event, the shared phase contour closes, and a single future-directed null continuation emerges. Creation and annihilation are time-reversed navigations of the same topological seam.
...
Crucially, CPT symmetry is *preserved locally*. The local CPT theorem remains valid at every point in spacetime: a full rotation in the local orientation bundle (C × P × T) returns the system to its original state. Charge conjugation C is realised as a local reflection in the orientation bundle (reversing the direction of proper-time phase accumulation), not as a global sheet transition. The global topology of the temporal manifold is two-sheeted: a particle and its antiparticle reside on opposite sheets, but this separation is a consequence of the global boundary conditions, not of the local C operation itself. This construction is not a substitute for the screened QED calculation of pair-production amplitudes; it supplies the proposed TEP geometric interpretation of the same laboratory process. This suggests a possible topological origin for the observed matter-antimatter asymmetry: rather than requiring a symmetry-breaking event in the early universe, the dominance of matter may reflect the global topology of the temporal manifold, which may favor one sheet over the other in certain density regimes. A quantitative prediction of the baryon-to-photon ratio from this topological bias, evaluated via MCMC parameter estimation, is developed in TEP-C0 (Paper 26, Athens).
...
- *Spin and antimatter* are proposed to admit geometric reinterpretations as orientations on the two-sheeted proper-time manifold, rather than being treated as primitive internal quantum properties. Antimatter is the reversed proper-time phase orientation on the second sheet; CPT is preserved locally while the global two-sheeted topology isolates matter and antimatter topologically.
