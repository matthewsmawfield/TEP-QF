...
The first-principles finite-core lattice solver (`scripts/steps/step_06_finite_core_lattice.py`) replaces the uniform mean-field source with a periodic array of Gaussian topological charge cores (width r<sub>c</sub>) at the physical electron spacing a = n<sub>e</sub><sup>-1/3</sup>, and solves the screened Klein-Gordon equation by FFT. The solver uses k-space superposition of core profiles and is scaled to 4<sup>3</sup>, 8<sup>3</sup>, and 16<sup>3</sup> microcells to test convergence.

At bulk density &rho; = 20 g/cm<sup>3</sup>, the coherent lattice response suppresses the volume-averaged scalar field to a fixed fraction of the mean-field value:

|<&phi;><sub>lattice</sub>| / |<&phi;><sub>MF</sub>| = 0.25 &plusmn; 0.01,

independent of microcell size from 4<sup>3</sup> to 16<sup>3</sup> cores. This defines an effective source density &rho;<sub>eff</sub> = &rho; |<&phi;><sub>lattice</sub>|/|<&phi;><sub>MF</sub>| &approx; 5 g/cm<sup>3</sup> at &rho; = 20 g/cm<sup>3</sup>, within a factor of four of the phenomenological saturation scale &rho;<sub>T</sub> &approx; 20 g/cm<sup>3</sup>. Random-phase ensembles in the 4<sup>3</sup> microcell yield f<sub>RP</sub> &approx; 0.22, consistent with partial decoherence at the 1/&radic;N<sub>micro</sub> level (1/&radic;64 &approx; 0.13). The residual gap to &rho;<sub>T</sub> is attributed to decoherence over the full terrestrial coherence volume N<sub>eff</sub> &sim; 10<sup>50</sup>, which cannot be simulated directly but is bounded by the random-phase prediction 1/&radic;N<sub>eff</sub> &sim; 10<sup>-25</sup> combined with the finite-core geometric factor. Figure 7 shows the &rho;<sub>eff</sub> mapping and the lattice-size scaling study.

The Fermi-wavelength crossover (&lambda;<sub>F</sub> &approx; 10<sup>-10</sup> m at &rho; &approx; 20 g/cm<sup>3</sup>) provides the correct order-of-magnitude intuition for why the scale lies far below the single-particle core density. The exact closure of the remaining factor-of-four gap is the next target: compounding the microcell lattice suppression with the coherence-volume random-phase factor from the I<sub>0</sub> Bessel derivation.
